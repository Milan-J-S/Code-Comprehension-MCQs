...
for item in comments:
    ...
    for i in range(10):
        ...
        if(i < len(comment)):
            comment_tensor[comments_vector[comment[i]]+1] = 1
        else:
            comment_tensor[0] = 1
...
hidden = Embedding(round(len(all_codes_tensor)*1.3), 32 )(inp)
hidden = LSTM(128)(hidden)
hidden = Dropout(0.5)(hidden)
# hidden = LSTM(256 )(hidden)
# hidden = Dropout(0.5)(hidden)
out1 = Dense(len(vocab)+1)(hidden)
out2 = Dense(len(vocab)+1)(hidden)
out3 = Dense(len(vocab)+1)(hidden)
out4 = Dense(len(vocab)+1)(hidden)
out5 = Dense(len(vocab)+1)(hidden)
out6 = Dense(len(vocab)+1)(hidden)
out7 = Dense(len(vocab)+1)(hidden)
out8 = Dense(len(vocab)+1)(hidden)
out9 = Dense(len(vocab)+1)(hidden)
out10 = Dense(len(vocab)+1)(hidden)
...
model.fit(np.asarray(codes), tensor, epochs=100, verbose = 1, batch_size=5, validation_split=0.05)

model.save("separate.h5")