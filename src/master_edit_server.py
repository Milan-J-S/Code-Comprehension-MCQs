...
def showCode():
    ...
    if random.random() < 0.5:
        fileToRender =  'createDistractorsEdit.html'
    else:
        fileToRender = 'fixDistractorsEdit.html'
...
def changeComment():
    ...
    originalComment =  payload.get('originalComment',[()])[0]
    ...
    cur.execute("Update Comments SET comment = (?) where comment = (?)", (comment, originalComment))
    cur.execute("Update CodeComments SET comment = (?) where comment = (?) AND code = (?)", (comment, originalComment, filename))
    con.commit()
